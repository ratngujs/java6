...
<body>
	...
	<table>
		<tr>
			<td style="width: 150px">Người mua hàng</td>
		...
		<tr>
			...
			<td></td>
			<td>
				<c:if test="${not order.status && not order.cancel}">
					<a href="#xacnhan" class="btn btn-primary" data-id="${order.id}">Xác nhận</a>
					<a href="#huydon" class="btn btn-danger" data-id="${order.id}">Hủy đơn hàng</a>
				</c:if>
	...
	<script>
		$(document).ready(function(){
			...
			$("a[href='#xacnhan']").on("click", function(evt){
				let idOrder = $(this).data("id");
